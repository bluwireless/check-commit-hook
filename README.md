...
The script uses a default configuration file located in `bw_check_commit_hook/data/checkpatch.yaml` but it can be overriden by a custom configuration file and passed to the script using the `--config-file` option.
...
The `all` key can be used to apply the same configuration to all the files in the repository.
...
The `ERRORS_ENABLED` key can be used as a placeholder for all the errors enabled for all the files, and this needs to be part of `errors_enabled` list to be used.
